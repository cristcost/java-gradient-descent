...
class SgdWithMomentumOptimizerTest {
  ...
  void basicTest() {
    ...
    Optimizer optimizer = new SgdWithMomentumOptimizer(0.1, 0.9);
    ...
    assertThat(parameters[0]).isLessThan(lastParameters[0]); // "Less than" due to momentum
    ...
    assertThat(parameters[0]).isLessThan(lastParameters[0]); // "Less than" due to momentum
    assertThat(parameters[1]).isLessThan(lastParameters[1]); // "Less than" due to momentum
  ...
  void basicConvergence() {
    ...
    Optimizer optimizer = new SgdWithMomentumOptimizer(0.1, 0.01);

    // Not a real gradient, just +1 or -1 if axis bigger or smaller than target
    double[] pseudoGradient = new double[3];
    int i = 0;

    while (i <= 100 && RawMeanSquareError.compute(parameters, target) > 0.01) {
